...
typedef enum IfcNames { NoInterface, IfcNames_MemServerRequestS2H, IfcNames_MMURequestS2H, IfcNames_MemServerIndicationH2S, IfcNames_MMUIndicationH2S, IfcNames_DmaIndicationH2S0, IfcNames_DmaIndicationH2S1, IfcNames_DmaRequestS2H0, IfcNames_DmaRequestS2H1, IfcNames_LoopbackControlS2H,  } IfcNames;
...
enum { CHAN_NUM_LoopbackControl_loopback,CHAN_NUM_LoopbackControl_marker};
#define LoopbackControl_reqinfo 0x20008

typedef struct {
    int lb;
} LoopbackControl_loopbackData;
typedef struct {
    uint32_t fb;
} LoopbackControl_markerData;
typedef union {
    LoopbackControl_loopbackData loopback;
    LoopbackControl_markerData marker;
} LoopbackControlData;
int LoopbackControl_handleMessage(struct PortalInternal *p, unsigned int channel, int messageFd);
typedef struct {
    PORTAL_DISCONNECT disconnect;
    int (*loopback) (  struct PortalInternal *p, const int lb );
    int (*marker) (  struct PortalInternal *p, const uint32_t fb );
} LoopbackControlCb;
extern LoopbackControlCb LoopbackControlProxyReq;
...
extern LoopbackControlCb LoopbackControlJsonProxyReq;
